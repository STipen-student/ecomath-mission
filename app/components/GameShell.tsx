...
export function GameShell({ caseData, modeStatus, onReset, onNewCase, nextActionLabel = "Generate kasus baru →", missionMeta, studentName, onMissionComplete }: { caseData: CityCase; modeStatus: string; onReset: () => void; onNewCase: () => void; nextActionLabel?: string; missionMeta?: { current: number; total: number; completed: number }; studentName?: string; onMissionComplete?: (report: MissionReport) => void | Promise<void> }) {
  ...
  return <main className="game-page pixel-game-theme"><header className="game-header"><button className="brand-button" onClick={onReset}><span className="brand-mark"><i /><b /><em /></span><span><strong>EcoMath Mission</strong><small>Sustainability City Challenge</small></span></button><div className="mission-title"><small>{missionMeta ? `DISTRIK ${missionMeta.current}/${missionMeta.total}` : "MISSION CONTROL"}</small><strong>{caseData.cityName}</strong><span>{caseData.difficulty}</span></div><div className="header-actions">{studentName && <span className="student-playing">♟ {studentName}</span>}{missionMeta && <span className="campaign-progress-chip">🏁 {missionMeta.completed}/{missionMeta.total} selesai</span>}<span className="mode-status"><i />{modeStatus}</span><button onClick={onReset}>🗺 Peta misi</button><button onClick={replay}>Reset distrik</button></div></header>
    ...
    <div className="ecd-score-strip"><span>REPUTASI</span>{scores.map((score, index) => <i className={score !== null ? "filled" : ""} key={index} />)}<strong>{total}<small>/100</small></strong></div>
    <div className="game-layout"><div className="game-main"><aside className="case-ribbon"><span className="difficulty">{missionMeta ? `QUEST ${missionMeta.current}` : caseData.difficulty}</span><div><small>{caseData.initialCondition}</small><strong>{caseData.variables[0].name} (x) + {caseData.variables[1].name} (y)</strong></div>{caseData.sdgFocus.map((sdg) => <span className="sdg-pill" key={sdg}>{sdg.split("·")[0]}</span>)}</aside>
    ...
    {complete && <ScoreSummary scores={scores.map((score) => score ?? 0)} onReplay={replay} onNewCase={onNewCase} nextActionLabel={nextActionLabel} missionLabel={missionMeta ? `Distrik ${missionMeta.current}/${missionMeta.total}` : undefined} />}
  </main>;
}
