...
export function MathPlanningPanel({ caseData, onComplete }: { caseData: CityCase; onComplete: (score: number, evidence: string[]) => void }) {
  ...
  return (
    <section className="phase-card">
      ...
      <div className="intel-equation-panel"><div><span className="kicker blue">HASIL INTEL SCAN</span><h2>Resource kota yang harus diterjemahkan ke blueprint</h2><p>Intel ini berasal dari kartu fakta penting. Gunakan sebagai petunjuk untuk mengisi koefisien, operator, dan batas pada rule engine di bawah.</p></div><div className="intel-equation-grid">{caseData.constraints.map((constraint) => <article className={`intel-equation-card ${constraint.type}`} key={constraint.id}><span className={`metric-icon ${constraint.type}`}>{constraint.type === "budget" ? "Rp" : constraint.type === "land" ? "▦" : constraint.type === "service" ? "☺" : "CO₂"}</span><div><small>{constraint.label}</small><strong>{constraint.a}x + {constraint.b}y {symbol(constraint.op)} {constraint.limit}</strong><p>{constraint.explanation}</p></div><b>{constraint.unit}</b></article>)}</div></div>
